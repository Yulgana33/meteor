...
_.extend(SynchronousCursor.prototype, {
  ...
  // Returns the next object (like with _nextObject) or throws if it doesn't
  // return within timeoutMS ms.
  _nextObjectWithTimeout: function (timeoutMS) {
    const self = this;
    if (!timeout) {
    ...
    const timeoutPromise = new Promise((resolve, reject) => {
      const timer = setTimeout(() => {
        reject(new Error('Client-side timeout waiting for next object'));
        self.close();
      }, timeoutMS);
    });
    return Promise.race([nextObjectPromise, timeoutPromise]).wait();
  },
  ...
  var loop = function () {
    ...
    while (true) {
      if (stopped)
        return;
      try {
        doc = cursor._nextObjectWithTimeout(timeoutMS);
      } catch (err) {
      ...
      // Since cursor._nextObjectWithTimeout can yield, we need to check again
      // to see if we've been stopped before calling the callback.
      if (stopped)
        return;
      if (doc) {
